...
const ThankYouModal: React.FC<Props> = ({ voterName, actorId, voterCount, phoneNumber, onClose }) => {
  ...
  // FUNCIÓN PARA COMPARTIR CARNET REAL (IMAGEN) DIRECTO A WHATSAPP
  const handleShareDirectly = async () => {
    ...
    try {
      const canvas = await html2canvas(cardRef.current!, {
        scale: 3, // Alta calidad para el carnet
        useCORS: true,
        backgroundColor: '#ffffff',
      });

      const blob = await new Promise<Blob | null>(resolve => canvas.toBlob(resolve, 'image/png'));
      if (!blob) throw new Error("Fallo generación carnet");

      const file = new File([blob], `Victoria_102_N${voterCount}.png`, { type: 'image/png' });

      // Mensaje que acompaña la foto
      ...
      // Intentar compartir archivo real (WhatsApp lo soporta perfectamente en móviles)
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        // Fallback: Descarga directa y link de texto
      ...
      alert("Hubo un error al generar el envío automático. Por favor, descarga la imagen.");
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-[#020617]/98 backdrop-blur-3xl animate-in fade-in duration-500 overflow-y-auto">
        ...
        <div className="relative mx-auto" style={{ maxWidth: '440px' }}>
          ...
          >
            <div className="p-8 pb-12 text-center space-y-10 relative">
              ...
              <div className="bg-[#1e3a8a] py-16 px-6 rounded-[3rem] shadow-2xl relative mt-4 overflow-hidden">
                 ...
                 <div className="relative z-10 flex flex-col items-center gap-2">
                    <div className="text-white font-black text-6xl italic tracking-tighter flex items-center gap-5">
                      NÚMERO 
                      <span className="text-[#facc15] text-[7.5rem] drop-shadow-[0_15px_30px_rgba(0,0,0,0.6)] non-italic leading-none">
                        {voterCount}
                      </span>
                    </div>
              ...
              <div className="space-y-10 px-2">
                ...
                <div className="bg-slate-50/90 p-12 rounded-[3.5rem] border border-slate-100 relative shadow-inner">
              ...
              <div className="flex flex-col items-center pt-4">
                 <div className="bg-[#1e3a8a] py-6 px-14 rounded-[2rem] shadow-3xl flex items-center gap-10 border-b-8 border-black/20">
            ...
            <div className="bg-[#facc15] py-5 text-center">
              <span className="text-[#1e3a8a] font-black text-[13px] uppercase tracking-[0.6em] italic">#UNIDOS POR TI BOYACÁ</span>
            </div>
          </div>
          ...
          <div className="mt-12 space-y-5 px-4">
            <button 
              ...
              className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-black py-8 rounded-[3rem] transition-all shadow-[0_20px_60px_rgba(16,185,129,0.5)] active:scale-95 text-2xl uppercase tracking-widest flex items-center justify-center gap-6 disabled:opacity-50"
            >
              <i className={`fa-solid ${isProcessing ? 'fa-spinner fa-spin' : 'fa-share-nodes'} text-4xl`}></i>
              {isProcessing ? 'PROCESANDO...' : 'ENVIAR CARNET A WHATSAPP'}
            </button>

            <button 
              onClick={onClose}
              className="w-full bg-amber-400 hover:bg-amber-500 text-blue-950 font-black py-7 rounded-[3rem] transition-all shadow-xl text-xl uppercase tracking-widest flex items-center justify-center gap-4 border-2 border-white/20"
            >
